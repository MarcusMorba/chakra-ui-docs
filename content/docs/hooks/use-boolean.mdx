...
function Example() {
  ...
  return (
    <>
      ...
      <button onmouseup={setFlag.toggle}>
...
function Example() {
  ...
  return (
    <button onMouseEnter={setFlag.on} onMouseLeave={setFlag.off} onfocusin={setFlag.on} onfocusout={setFlag.off}>
      {flag ? 'The flag is ON!' : 'Hover or focus me to turn ON'}
    </button>
  )
}
